...
interface MutTable<T, TKey extends keyof T, TKeyType = T[TKey], TInsert = InsertType<T, TKey>> {
    /**
     ...
     */
    delete(keys: TKeyType | TKeyType[], cascadeAction: CascadeAction): Promise<void>;
    /**
...
export const MUT_ITEMS: MutTable<Item, "id"> = {
    delete: async (keys, cascadeAction = "update") => {
        ...
        const deletedItems: Map<ItemId, Item> = cascadeAction !== "none"
            ? await db.items.bulkGet(keys).then(items => items.filter(i => i != null).reduce((map, item) => map.set(item.id, item), new Map<ItemId, Item>()))
            : new Map();
        const children = cascadeAction !== "none"
            ? await db.invItems.where("itemId").anyOf(keys).toArray()
            : [];

        if (cascadeAction === "delete") {
            await MUT_INV_ITEMS.delete(children.map(invItem => invItem.id), cascadeAction);
